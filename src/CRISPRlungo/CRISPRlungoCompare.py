...
def compareCRISPRlungo(arg_els):
    ...
    parser.add_argument('--suppress_log_scale', help='Suppress log scale for read count comparison', action='store_true')
    ...
    plot_root = '%s.%s_vs_%s' % (args.output_root, f1_name, f2_name)
    plot_tx_list(f1_final_cut_counts, f1_cut_annotations, f1_cut_classification_lookup, f2_final_cut_counts, plot_root, f1_name=f1_name, f2_name=f2_name, suppress_log_scale=args.suppress_log_scale)

    plot_root = '%s.%s_vs_%s' % (args.output_root, f2_name, f1_name)
    plot_tx_list(f2_final_cut_counts, f2_cut_annotations, f2_cut_classification_lookup, f1_final_cut_counts, plot_root, f1_name=f2_name, f2_name=f1_name, suppress_log_scale=args.suppress_log_scale)
    ...
    column_heads = ['total_input_reads', 'post_dedup_reads', 'post_primer_filter_reads', 'post_quality_filter_reads',
                    'discarded_quality_filter_reads', 'Not primary alignment', 'Duplicate read', 'Not supported by R2', 'Poor alignment', 'analyzed_read_count']
...
def parse_frag_tx_list(filename):
    ...
    with open(filename, 'r') as fin:
        ...
        for line in fin:
            ...
            cut_annotations[tx_cut] = cut_annotation
...
def plot_tx_list(f1_final_cut_counts, f1_cut_annotations, f1_cut_classification_lookup, f2_final_cut_counts,
                 plot_name, f1_name=None, f2_name=None, suppress_log_scale=False):
    """
    ...
    Args:
        ...
        plot_name: root of plot (.txt for data and .png and .pdf will be added)
        f1_name: name of f1 to display at top of plot
        f2_name: name of f2 to display at top of plot
        suppress_log_scale: if true, counts will be plotted in linear scale
...
"""
    ...
    for tx_order_obj in f2_final_cut_counts:
        if tx_order_obj in top_sorted_tx_list:
    ...
    if len(top_sorted_tx_list) > 0:
        ...
        for tx_order_obj in top_sorted_tx_list:
            ...
            if this_chr_pos in f1_cut_annotations:
                this_cut_anno = f1_cut_annotations[this_chr_pos]
        ...
        if not suppress_log_scale:
            ...
            ax3.set_xlim(0, max(5, max(counts_2)))
    ...
    with open(tx_list_report, 'w') as fout:
        ...
        for tx_key in f1_final_cut_counts:
            cut_annotation = 'Novel (not annotated)'
            if tx_key in f1_cut_annotations:
                cut_annotation = f1_cut_annotations[tx_key]
            tx_classification = 'Unknown (not classified)'
